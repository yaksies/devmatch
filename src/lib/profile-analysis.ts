...
function buildFallbackInsight(profile: ReturnType<typeof normalizeProfile>, github: GithubProfile | null): ProfileInsight {
  ...
  const summaryParts = [
    profile.headline || `${profile.displayName} shared a profile with limited written detail.`,
    techs.length ? `Their core stack appears to include ${techs.join(", ")}.` : "They have not listed a detailed stack yet.",
    profile.projects ? `They also mentioned extra projects or experience: ${profile.projects.slice(0, 140)}${profile.projects.length > 140 ? "…" : ""}.` : null,
    github?.bio ? `Public GitHub bio: ${github.bio}.` : null,
